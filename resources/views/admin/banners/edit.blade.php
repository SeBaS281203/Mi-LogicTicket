...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-lg">
    ...
    <form method="POST" action="{{ route('admin.banners.update', $banner) }}" enctype="multipart/form-data" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" id="title" value="{{ old('title', $banner->title) }}" required class="w-full rounded-lg border-slate-300 shadow-sm">
            @error('title')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>
        <div>
            <label for="subtitle" class="block text-sm font-medium text-slate-700 mb-1">Subtítulo</label>
            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $banner->subtitle) }}" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
        <div>
            <label for="image" class="block text-sm font-medium text-slate-700 mb-1">Nueva imagen (opcional)</label>
        ...
        <div>
            ...
            <input type="url" name="link_url" id="link_url" value="{{ old('link_url', $banner->link_url) }}" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
        <div>
            <label for="link_text" class="block text-sm font-medium text-slate-700 mb-1">Texto del botón</label>
            <input type="text" name="link_text" id="link_text" value="{{ old('link_text', $banner->link_text) }}" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
                <label for="starts_at" class="block text-sm font-medium text-slate-700 mb-1">Vigencia desde</label>
                <input type="date" name="starts_at" id="starts_at" value="{{ old('starts_at', $banner->starts_at?->format('Y-m-d')) }}" class="w-full rounded-lg border-slate-300 shadow-sm">
            </div>
            <div>
                <label for="ends_at" class="block text-sm font-medium text-slate-700 mb-1">Vigencia hasta</label>
                <input type="date" name="ends_at" id="ends_at" value="{{ old('ends_at', $banner->ends_at?->format('Y-m-d')) }}" class="w-full rounded-lg border-slate-300 shadow-sm">
            </div>
        </div>
        <div>
            <label for="sort_order" class="block text-sm font-medium text-slate-700 mb-1">Orden</label>
            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $banner->sort_order) }}" min="0" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
        <div class="flex items-center">
            <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $banner->is_active) ? 'checked' : '' }} class="rounded border-slate-300">
