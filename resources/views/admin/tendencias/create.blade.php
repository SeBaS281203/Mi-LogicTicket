...
@section('title', 'Nueva tendencia')

@section('content')
<h1 class="text-3xl font-bold mb-6">Nueva publicidad - Nuestras Tendencias</h1>

<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-lg">
    <form method="POST" action="{{ route('admin.tendencias.store') }}" enctype="multipart/form-data" class="space-y-4">
        @csrf
        <div>
            <label for="titulo" class="block text-sm font-medium text-slate-700 mb-1">Título (opcional)</label>
            <input type="text" name="titulo" id="titulo" value="{{ old('titulo') }}" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
        <div>
            <label for="imagen" class="block text-sm font-medium text-slate-700 mb-1">Imagen <span class="text-red-500">*</span></label>
            <input type="file" name="imagen" id="imagen" accept="image/*" required class="w-full rounded-lg border-slate-300 shadow-sm">
        ...
        <div>
            ...
            <input type="url" name="link" id="link" value="{{ old('link') }}" class="w-full rounded-lg border-slate-300 shadow-sm" placeholder="https://">
        </div>
        ...
        <div>
            ...
            <input type="number" name="orden" id="orden" value="{{ old('orden', 0) }}" min="0" class="w-full rounded-lg border-slate-300 shadow-sm">
        </div>
